...
int interval = 200; // every 2 seconds
...
void loop() {
  ...
  if (wifiConnected) {
    if (udpConnected) {
      // Currently not sending "beat" or button message,
      // only digital value and analog read (see below):
      /*
            ...
            // check button and send if changed to low
            buttonState = digitalRead(buttonPin);

            if (buttonState == LOW && buttonState != lastButtonState) {
              // when button is pressed, send a message to the first IP address:
